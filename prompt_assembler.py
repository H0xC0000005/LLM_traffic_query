...
import re
import pathlib
from typing import Any, Dict, List, Mapping, Optional, Tuple, Protocol
import yaml

import hashlib

from scene_preprocessor import BasicStatsPreprocessor, ScenePreprocessor
...
class PromptAssembler:
    def __init__(self, template_path: pathlib.Path,
                 ...
                 preprocessor: Optional[ScenePreprocessor] = None):
        ...
        self.query_base = self._get_path(["query_prompt", "templates", "base"])
        self.response_templates = self._get_path(["response_prompt", "templates"]) or {}

        self.preprocessor = preprocessor
        self._precompute_cache: Dict[str, Dict[str, str]] = {}  # key: sha1(scene_yaml_text)
    ...
    # Helpers
    # ---------------------------------------------------------
    # NEW: stable hash for cache keys
    def _sha1_text(self, s: str) -> str:
        return hashlib.sha1(s.encode("utf-8")).hexdigest()

    # NEW: convert preprocessor outputs to text symbols and inject into a symbol dict
    def _inject_precomputed(self, base_symbols: Dict[str, str], scene_yaml_text: str, scene_obj: Optional[dict] = None) -> None:
        if not self.preprocessor:
            return
        key = self._sha1_text(scene_yaml_text)
        if key in self._precompute_cache:
            base_symbols.update(self._precompute_cache[key])
            return

        if scene_obj is None:
            try:
                scene_obj = yaml.safe_load(scene_yaml_text) or {}
            except Exception:
                scene_obj = {}

        produced = self.preprocessor.run(scene_obj) or {}
        as_text: Dict[str, str] = {}
        for k, v in produced.items():
            if isinstance(v, str):
                as_text[k] = v
            else:
                as_text[k] = dump_yaml(v)  # serialize dict/list/etc. to YAML text
        # cache and inject
        self._precompute_cache[key] = as_text
        base_symbols.update(as_text)

    # NEW: a public helper if you want to set/replace the current scene on the assembler
    def set_scene_from_path(self, scene_yaml_path: pathlib.Path) -> None:
        data = read_yaml(scene_yaml_path)
        txt = dump_yaml(data)
        self.symbols["scenario"] = txt
        self._inject_precomputed(self.symbols, txt, data)
    ...
    # NEW:
    def _missing_resolver(self, symbols_ref: Optional[Dict[str, str]] = None) -> callable:
        def on_missing(name_expr: str) -> Optional[str]:
            # 0) Try exact dotted-path inside the template as-is
            node = get_by_dotted(self.template, name_expr.strip())
            if node is not None:
                return node_to_text(node)

            # 1) Parse base + attrs (supports '.' and '->' you added earlier)
            base, attrs = self._parse_name_expr(name_expr)

            # 2) Resolve 'base' from current symbols first (e.g., scenario set by caller)
            if symbols_ref and base in symbols_ref:
                val = symbols_ref[base]
            else:
                # 3) Then try inside the template by base name
                node_base = get_by_dotted(self.template, base)
                if node_base is not None:
                    val = node_to_text(node_base)
                else:
                    # 4) Finally try external resolver (e.g., free_flow_example.yaml)
                    val = self.resolver.maybe_load(base) if self.resolver is not None else None

            if val is None:
                return None

            # 5) Apply attribute chain (e.g., .vehicles)
            val = self._apply_attr_chain(val, attrs)

            # 6) (Optional) Post-process if you kept filters/implicit rules
    ...
    def render_initialization(self) -> str:
        ...
        base_symbols = dict(self.symbols)
        return render_template(self.init_base, base_symbols, on_missing=self._missing_resolver(base_symbols))


    def render_query_for_scene(self, scene_yaml_path: pathlib.Path) -> str:
        if not isinstance(self.query_base, str):
            raise ValueError("query_prompt.templates.<query|base> must be a string")
        scene_data = read_yaml(scene_yaml_path)
        scene_yaml_text = dump_yaml(scene_data)
        symbols = dict(self.symbols)
        symbols["scenario"] = scene_yaml_text
        # PRECOMPUTE here into local symbols (e.g., scene_stats, anything else)
        self._inject_precomputed(symbols, scene_yaml_text, scene_data)
        return render_template(self.query_base, symbols, on_missing=self._missing_resolver(symbols))
    ...
    def render_response(self, kind: str, *, details: str = "") -> str:
        ...
        if details:
            symbols["details"] = details
        return render_template(tmpl, symbols, on_missing=self._missing_resolver(symbols))
...
def main():
    ...
    with out_path.open("w", encoding="utf-8") as f, \
        ...
        try:
            ...
            assembler = PromptAssembler(template_path, resolver=resolver, preprocessor=BasicStatsPreprocessor(),)
