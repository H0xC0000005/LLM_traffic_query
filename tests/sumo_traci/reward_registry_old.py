...
- The registry stores import paths as strings, so literature rewards can live outside
  ``utility.py`` / ``utility_portable_snapshot.py``.
- ``resolve_reward(...)`` returns the constructed callable directly, as requested.
...
# Edit these paths as your project structure evolves.
# Using string targets keeps literature rewards out of utility.py when you add them later.
...
def resolve_reward(reward_name: str, **bound_kwargs: Any) -> RewardCallable:
    ...
    ----------
    reward_name:
        Key from the reward registry.
    **bound_kwargs:
        Experiment-level configuration captured once before training starts.

    Returns
    -------
    RewardCallable
        A callable suitable for the centralized trainer wrapper.
    """
    try:
