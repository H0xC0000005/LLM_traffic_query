...
def main() -> None:
    ...
    # Drop identity columns as requested (for training inputs; still can be present in df for auditing)
    drop_cols = ["timestamp"]  # timestamp used only for split; not a feature
    ...
    if args.use_expert_features:
        ...
        if expert_df is None:
            ...
            # Drop non-feature columns if extractor accidentally included them.
            ...
            # Avoid name collisions: keep baseline columns, rename colliding expert columns.
    ...
    # Time split (after expert features are appended)
    ...
    utility.common_save_json(
        {"feature_columns": feature_list, "categorical_columns": cat_present}, model_out_dir / "feature_manifest.json"
    )
