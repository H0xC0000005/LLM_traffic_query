...
def _sample_to_program(
    ...
    if version_generated is not None:
        body = code_manipulation.rename_function_calls(
            code=body, source_name=f"{function_to_evolve}_v{version_generated}", target_name=function_to_evolve
        )
...
class LocalSandbox(Sandbox):
    ...
    def run(
        ...
        **kwargs,
        ...
        results = self._compile_and_run_function(
            program, function_to_run, function_to_evolve, dataset, self._numba_accelerate, result_container
        )
    ...
    def _compile_and_run_function(
        self, program, function_to_run, function_to_evolve, dataset, numba_accelerate, result_container
    ):
        try:
...
class Evaluator:
    ...
    def analyse(
        ...
        **kwargs,
        ...
        candidate_function_source = str(new_function)
        candidate_program_source = program

        scores_per_test = {}
        time_reset = time.time()
        self._inputs["data"]["inputs"] = data_input
        self._inputs["data"]["outputs"] = data_output
        self._inputs["data"]["_llmfe_candidate_function_source"] = candidate_function_source
        self._inputs["data"]["_llmfe_candidate_program_source"] = candidate_program_source
