...
def _fit_quantile_map(
    ...
    *,
    ...
    # NEW IMPLEMENTATION:
    # Approximate q05/q95 using grouped mean +/- z * std.
    # This keeps the broad “lower/upper operating envelope” meaning while avoiding
    # the very expensive exact grouped quantile computation and string-key aggregation.
    work = df[group_cols + [target_col]].copy().dropna(subset=group_cols + [target_col])
    if work.empty:
        mapping_df = pd.DataFrame(columns=[*group_cols, "_q_proxy"])
        return {
    ...
    grouped = work.groupby(group_cols, sort=False)[target_col].agg(["mean", "std"]).reset_index()
    grouped["std"] = grouped["std"].fillna(0.0)

    z = 1.6448536269514722  # approx 95th percentile of N(0, 1)
    if q >= 0.5:
        grouped["_q_proxy"] = grouped["mean"] + z * grouped["std"]
    else:
        grouped["_q_proxy"] = grouped["mean"] - z * grouped["std"]

    mapping_df = grouped[group_cols + ["_q_proxy"]].copy()

    global_mean = float(work[target_col].mean())
    global_std = float(work[target_col].std(ddof=0)) if len(work) > 1 else 0.0
    global_default = global_mean + z * global_std if q >= 0.5 else global_mean - z * global_std
...
def _apply_quantile_map(
    ...
    *,
    ...
    # NEW IMPLEMENTATION:
    # Vectorized merge on the original group columns; avoids row-wise string key creation.
    mapping_df = state["mapping_df"]
    if mapping_df.empty:
        values = np.full(len(df), state["global_default"], dtype=np.float32)
        return pd.DataFrame({output_col: values}, index=df.index)

    merged = df[group_cols].merge(mapping_df, on=group_cols, how="left", sort=False)
    values = merged["_q_proxy"].fillna(state["global_default"]).to_numpy(dtype=np.float32, copy=False)
    return pd.DataFrame({output_col: values}, index=df.index)
...
def extract_rank5_mma_features(
    ...
    **_: Any,
    ...
    Implemented as one coherent feature block with:
      ...
      - optional building-meter lower/upper envelope proxy features.

    Quantile proxies are now computed by grouped mean/std approximation for speed.
    ...
    # NEW IMPLEMENTATION:
    # Avoid copying the full input frame; only materialize what is needed.
    if mode == "fit_transform":
        if label_col not in df.columns:
            raise ValueError(f"'{label_col}' is required in mode='fit_transform'")
        target = np.log1p(pd.to_numeric(df[label_col], errors="coerce").fillna(0.0).clip(lower=0.0))
        fit_df = base.copy()
        if use_building:
            fit_df[building_col] = df[building_col].astype(str)
        fit_df[primary_use_col] = df[primary_use_col].astype(str) if primary_use_col in df.columns else "Unknown"
        fit_df[meter_col] = (
            pd.to_numeric(df[meter_col], errors="coerce").fillna(0).astype(np.int16) if meter_col in df.columns else 0
        )
        ...
        meta["frac_primary_use_hour"] = _fit_ratio_map(
            fit_df, entity_col=primary_use_col, bucket_col="r5_local_hour", target_col=label_col
        )
        meta["frac_primary_use_weekday"] = _fit_ratio_map(
            fit_df, entity_col=primary_use_col, bucket_col="r5_local_weekday", target_col=label_col
        )
        meta["frac_primary_use_dayofyear"] = _fit_ratio_map(
            fit_df, entity_col=primary_use_col, bucket_col="r5_local_dayofyear", target_col=label_col
        )

        if use_building:
            print(f"[rank5]: Fitting rank5_mma building features with {len(fit_df)} rows...")
            ...
            meta["frac_building_hour"] = _fit_ratio_map(
                fit_df, entity_col=building_col, bucket_col="r5_local_hour", target_col=label_col
            )
            meta["frac_building_weekday"] = _fit_ratio_map(
                fit_df, entity_col=building_col, bucket_col="r5_local_weekday", target_col=label_col
            )
            meta["frac_building_dayofyear"] = _fit_ratio_map(
                fit_df, entity_col=building_col, bucket_col="r5_local_dayofyear", target_col=label_col
            )
            meta["building_meter_q95"] = _fit_quantile_map(
                fit_df, group_cols=[building_col, meter_col], target_col=label_col, q=0.95
            )
            meta["building_meter_q05"] = _fit_quantile_map(
                fit_df, group_cols=[building_col, meter_col], target_col=label_col, q=0.05
            )
        else:
            print("[rank5]: Skipping building features due to exclude_building_id=True or missing building_id column.")
            for key in [
                "frac_building_hour",
        ...
        use_building = (
            (not meta.get("exclude_building_id", False))
            and (building_col in df.columns)
            and ("frac_building_hour" in meta)
        )
    ...
    parts.extend(
        [
            _apply_ratio_map(
                feat,
                entity_col=primary_use_col,
                bucket_col="r5_local_hour",
                state=meta["frac_primary_use_hour"],
                output_col="r5_frac_primary_use_hour",
                min_count_for_exact=min_count_for_exact,
            ),
            _apply_ratio_map(
                feat,
                entity_col=primary_use_col,
                bucket_col="r5_local_weekday",
                state=meta["frac_primary_use_weekday"],
                output_col="r5_frac_primary_use_weekday",
                min_count_for_exact=min_count_for_exact,
            ),
            _apply_ratio_map(
                feat,
                entity_col=primary_use_col,
                bucket_col="r5_local_dayofyear",
                state=meta["frac_primary_use_dayofyear"],
                output_col="r5_frac_primary_use_dayofyear",
                min_count_for_exact=min_count_for_exact,
            ),
        ]
    )
    ...
    if use_building:
        parts.extend(
            [
                _apply_ratio_map(
                    feat,
                    entity_col=building_col,
                    bucket_col="r5_local_hour",
                    state=meta["frac_building_hour"],
                    output_col="r5_frac_building_hour",
                    min_count_for_exact=min_count_for_exact,
                ),
                _apply_ratio_map(
                    feat,
                    entity_col=building_col,
                    bucket_col="r5_local_weekday",
                    state=meta["frac_building_weekday"],
                    output_col="r5_frac_building_weekday",
                    min_count_for_exact=min_count_for_exact,
                ),
                _apply_ratio_map(
                    feat,
                    entity_col=building_col,
                    bucket_col="r5_local_dayofyear",
                    state=meta["frac_building_dayofyear"],
                    output_col="r5_frac_building_dayofyear",
                    min_count_for_exact=min_count_for_exact,
                ),
                _apply_quantile_map(
                    feat,
                    group_cols=[building_col, meter_col],
                    state=meta["building_meter_q95"],
                    output_col="r5_building_meter_q95_log1p",
                ),
                _apply_quantile_map(
                    feat,
                    group_cols=[building_col, meter_col],
                    state=meta["building_meter_q05"],
                    output_col="r5_building_meter_q05_log1p",
                ),
            ]
        )
