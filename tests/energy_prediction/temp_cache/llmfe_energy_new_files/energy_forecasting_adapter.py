...
IDENTIFIER_AND_TIME_COLUMNS: List[str] = [
    "timestamp",
    "building_id",
    "site_id",
    "pair_id",
...
class EnergyForecastingConfig:
    ...
    forbidden_output_cols: Tuple[str, ...] = tuple(IDENTIFIER_AND_TIME_COLUMNS + ["meter_reading"])
...
def _safe_worst_score(reason: str) -> CandidateScoreResult:
...
def assert_rowwise_candidate(
    ...
    for i in range(probe_n):
        ...
        for col in batch_out.columns:
            if not _series_equal(batch_out.loc[[i], col].reset_index(drop=True), single_out[col].reset_index(drop=True)):
                raise ValueError(
...
def _build_model_frame(
    ...
    categorical_cols = list(dict.fromkeys([c for c in config.base_categorical_cols if c in model_df.columns] + candidate_cat_cols))
...
def _train_lgbm_score(
    ...
    inner_eval_mask = ((day_index >= split.inner_eval_start_day) & (day_index < split.inner_eval_end_day_exclusive)).to_numpy()
    ...
    candidate_inner_tail = _apply_candidate_to_split(candidate_fn, inputs, inner_tail_mask, config, validate_rowwise=False)
    ...
    X_eval, y_eval, _ = prepare_tabular_matrices(
        eval_df, config.label_col, drop_cols=[], categorical_cols=cat_cols
    )
...
def _make_prompt_sample(
    ...
    input_cols = _candidate_input_columns(inputs, config)
    day_index = build_day_index(inputs, config.timestamp_col)
    train_mask = ((day_index >= config.split.train_start_day) & (day_index < config.split.train_end_day_exclusive)).to_numpy()
...
def score_candidate_feature(
    ...
    t0 = time.time()
    try:
        result = _train_lgbm_score(inputs.reset_index(drop=True), pd.Series(outputs).reset_index(drop=True), candidate_fn, cfg)
        result.runtime_seconds = time.time() - t0
    except Exception as exc:
        result = _safe_worst_score(str(exc))
        result.runtime_seconds = time.time() - t0
...
def score_candidate_from_llmfe(data: Mapping[str, Any], candidate_fn: CandidateFn) -> Tuple[float, pd.DataFrame, pd.Series]:
    ...
    result = score_candidate_feature(data["inputs"], data["outputs"], candidate_fn, cfg)
